...
export abstract class AbstractSearchDirective<T> implements OnInit, OnDestroy {
  ...
  private _sortSubject: BehaviorSubject<string> = new BehaviorSubject(null);
  ...
  params: IParams;
  formGroup: FormGroup;
  sort$: Observable<string> = this._sortSubject.asObservable().pipe(filter(sort => !!sort));
  ...
  getMore(): void {
    if (!this.route) {
      ...
      this.requestObservable(this.params).pipe(
        ...
        this._sortSubject.next(this.params.sort);
      });
    }
  }
