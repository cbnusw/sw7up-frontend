...
import { TopcitFiltersComponent } from './components/topcit-filters/topcit-filters.component';
...
export class ManagementTopcitPageComponent implements OnInit, OnDestroy, AfterViewInit {
  @ViewChild(TopcitFiltersComponent) filter: TopcitFiltersComponent;
    ...
    this.fileInputRef.nativeElement.addEventListener('change', (event) => {
      ...
      reader.onload = () => {
        ...
        try {
          ...
          const rows = sheetToJSON(topcitSheet);
          const stat = sheetToJSON(statSheet);
  ...
  private _mapTopcitData(rows: any[]): TopcitDto[] {
    return rows.map((row: any) => ({
      ...
      level: toNumber(row['수준']),
      totalScore: toNumber(row['총점']),
  ...
  private _mapStatData(rows: any[]): any[] {
    return rows.map((row: any) => ({
      ...
      totalScore: toNumber(row['총점']),
      subjects: Object.keys(row)
        .filter(key => key !== '순번' && !this._STAT_KEYS.includes(key))
        .map(key => {
          return ({
            name: key,
            score: toNumber(row[key])
